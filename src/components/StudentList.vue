  ...
  <div class="animate-in rounded-3xl p-6 border shadow-glow
    ...
    <div class="flex flex-col gap-4 mb-5">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center gap-2">
        ...
        <select
          ...
          class="rounded-xl px-3 py-2 border bg-[var(--bg)]/50 text-[var(--text)] text-sm
                 border-[var(--card-border)] focus:outline-none focus:ring-2
                 focus:ring-[var(--violet)]/40 focus:border-[var(--violet)]
    ...
    <div v-if="filteredStudents.length === 0" class="text-center py-14 animate-in">
      <div class="w-14 h-14 mx-auto mb-3 rounded-2xl bg-gradient-to-br from-[var(--violet)]/15 to-[var(--fuchsia)]/15
        ...
        <svg class="w-7 h-7 text-[var(--violet)]" fill="none" viewBox="0 0 24 24"
    ...
    <div v-else class="overflow-x-auto">
      <table class="w-full text-sm">
        <thead>
          <tr class="text-xs uppercase tracking-wide text-[var(--text-muted)]
            ...
            <th class="text-left font-medium py-2.5">Status</th>
        ...
        <TransitionGroup tag="tbody" name="list">
          <tr
            ...
            :key="student.id"
            class="border-b border-[var(--card-border)]/60
                   hover:bg-[var(--violet)]/[0.05]
          ...
          >
              ...
              <div class="flex items-center gap-3">
                <span
                  class="w-9 h-9 rounded-full flex items-center justify-center text-white
                         text-xs font-semibold shrink-0 bg-gradient-to-br shadow-md
            ...
            <td class="py-3 text-[var(--text)]">
              {{ student.course }}
            </td>

            <td class="py-3">
              <span class="text-xs px-2.5 py-1 rounded-full font-medium
                           bg-gradient-to-r from-[var(--violet)]/15
                           to-[var(--fuchsia)]/15 text-[var(--violet)]">
                {{ student.yearLevel }}{{ ordinalSuffix(student.yearLevel) }} Yr
              </span>
            </td>

            <td class="py-3 text-[var(--text-muted)]">
              {{ student.email }}
            </td>
